...
# Actually finding exactly where to put this
idx = content.rfind("}\n\nclass _OutletCard extends StatelessWidget {")
if idx == -1:
    idx = content.rfind("class _OutletCard extends StatelessWidget {")
    if idx != -1:
        # find the last closing brace before the class definition
        idx = content.rfind("}", 0, idx)

if idx != -1:
    content = content[:idx] + shimmer_methods + content[idx:]
