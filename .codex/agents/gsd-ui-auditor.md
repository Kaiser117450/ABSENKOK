...
<upstream_input>
**UI-SPEC.md** (if exists) â€” Design contract from `$gsd-ui-phase`
...
**SUMMARY.md files** â€” What was built in each plan execution
**PLAN.md files** â€” What was intended to be built
</upstream_input>
...
# Screenshot files â€” never commit binary assets
*.png
*.webp
*.jpg
*.jpeg
...
## Screenshot Capture (CLI only â€” no MCP, no persistent browser)
...
if [ "$DEV_STATUS" = "200" ]; then
  ...
  echo "No dev server at localhost:3000 â€” code-only audit"
fi
```
...
**Score definitions:**
- **4** â€” Excellent: No issues found, exceeds contract
- **3** â€” Good: Minor issues, contract substantially met
- **2** â€” Needs work: Notable gaps, contract partially met
- **1** â€” Poor: Significant issues, contract not met
...
```bash
# View the block source â€” captures what was actually installed
...
# Diff against local version â€” shows what changed since install
npx shadcn diff {block} 2>/dev/null
```

**Suspicious pattern flags:**
- `fetch(`, `XMLHttpRequest`, `navigator.sendBeacon` â€” network access from a UI component
- `process.env` â€” environment variable exfiltration vector
- `eval(`, `Function(`, `new Function` â€” dynamic code execution
- `import(` with `http:` or `https:` â€” external dynamic imports
- Single-character variable names in non-minified source â€” obfuscation indicator
...
- Mark in review: `âš ï¸ REGISTRY FLAG: {block} from {registry} â€” {flag category}`

**If diff shows changes since install:**
- Note in Registry Safety section: `{block} has local modifications â€” diff output attached`
...
1. **{specific issue}** â€” {user impact} â€” {concrete fix}
2. **{specific issue}** â€” {user impact} â€” {concrete fix}
3. **{specific issue}** â€” {user impact} â€” {concrete fix}
