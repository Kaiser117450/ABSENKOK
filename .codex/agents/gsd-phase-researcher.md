...
**AGENTS.md enforcement:** If `./AGENTS.md` exists, extract all actionable directives (required tools, forbidden patterns, coding conventions, testing rules, security requirements). Include a `## Project Constraints (from AGENTS.md)` section in RESEARCH.md listing these directives so the planner can verify compliance. Treat AGENTS.md directives with the same authority as locked decisions from CONTEXT.md â€” research should not recommend approaches that contradict them.
</project_context>
...
```bash
node "C:/Users/HYPE R Series/Desktop/projekan/absensi apk/absensi_enakko_flutter/.codex/get-shit-done/bin/gsd-tools.cjs" websearch "your query" --limit 10
```
...
|------------|------------|-----------|---------|----------|
| [tool] | [feature/requirement] | âœ“/âœ— | [version or â€”] | [fallback or â€”] |
...
1. **Extract external dependencies from phase description/requirements** â€” identify tools, services, CLIs, runtimes, databases, and package managers the phase will need.

2. **Probe availability** for each dependency:

```bash
# CLI tools â€” check if command exists and get version
command -v $TOOL 2>/dev/null && $TOOL --version 2>/dev/null | head -1

# Runtimes â€” check version meets minimum
...
# Databases / services â€” check if process is running or port is open
...
| Dependency | Required By | Available | Version | Fallback |
|------------|------------|-----------|---------|----------|
| PostgreSQL | Data layer | âœ“ | 15.4 | â€” |
| Redis | Caching | âœ— | â€” | Use in-memory cache |
| Docker | Containerization | âœ“ | 24.0.7 | â€” |
| ffmpeg | Media processing | âœ— | â€” | Skip media features, flag for human |

**Missing dependencies with no fallback:**
- {list items that block execution â€” planner must address these}

**Missing dependencies with fallback:**
- {list items with viable alternatives â€” planner should use fallback}
```

4. **Classification:**
   - **Available:** Tool found, version meets minimum â†’ no action needed
   - **Available, wrong version:** Tool found but version too old â†’ document upgrade path
   - **Missing with fallback:** Not found, but a viable alternative exists â†’ planner uses fallback
   - **Missing, blocking:** Not found, no fallback â†’ planner must address (install step, or descope feature)
...
```bash
node "C:/Users/HYPE R Series/Desktop/projekan/absensi apk/absensi_enakko_flutter/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
