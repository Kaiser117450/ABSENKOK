...
# Let's find exactly the end of the `_AdminOutletsScreenState` class
# I will find the first line after `Widget build(BuildContext context) {` block inside that class
# The class `_AdminOutletsScreenState` ends where the next top-level class begins, or right before `class _OutletSheet`
idx = content.find("\nclass _OutletSheet")
if idx != -1:
