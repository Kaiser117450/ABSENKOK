...
content = re.sub(
    r'Container\(\s*height: 70,\s*margin: const EdgeInsets\.only\(bottom: 8\),\s*decoration: BoxDecoration\(\s*color: Colors\.white,\s*borderRadius: BorderRadius\.circular\(14\),\s*boxShadow: \[\s*BoxShadow\(\s*color: Colors\.black\.withOpacity\(0\.04\),\s*blurRadius: 8,\s*offset: const Offset\(0, 2\),\s*\),\s*\],\s*\),\s*child:',
