-- ============================================================================
-- ABSENKOK — release migration bundle
-- ============================================================================
--
-- Combines the two migrations introduced in PRs #27 and #28 into a single
-- script for one-shot application. Run this in the Supabase SQL Editor as
-- a project owner. Both component scripts are idempotent (create … if not
-- exists, revoke … grant …) so re-running is safe.
--
-- This bundle is strictly ADDITIVE — it adds new tables, a new RPC, and
-- new RLS policies, but does not drop, rename, or tighten anything that
-- existing kiosk APKs depend on. See release/RELEASE_NOTES_TEMPLATE.md for
-- the backwards-compatibility guarantees.
-- ============================================================================
