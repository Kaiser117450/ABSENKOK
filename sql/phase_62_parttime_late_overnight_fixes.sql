-- Phase 62: PARTTIME late suppression + Overnight grace window
--
-- Fix 1 (PARTTIME no late): PARTTIME employees should never be flagged
--   as 'late' — they have flexible start times. Implemented by excluding
--   PARTTIME from has_late in the flags lateral.
--
-- Fix 2 (Overnight grace window): A shift on day D is NOT considered
--   'logical_day_complete' until 08:00 WITA on day D+1. This prevents
--   shift SORE workers (or any overnight worker) who clock out between
--   00:00–07:59 the next morning from being flagged as 'belum_absen_pulang'.
--   Applies to ALL outlets (not just TWENTY_FOUR_HOUR).
--
-- IMPORTANT: Review and apply manually in Supabase SQL Editor.
-- Helper functions are re-declared unchanged for completeness.

-- ─── Helper: unchanged from phase 57 ────────────────────────────────────────
...
-- ─── Main recap function: 3 targeted changes from phase 57 ───────────────────
-- CHANGE 1: attendance_day_rows.logical_day_complete — overnight grace window
-- CHANGE 2: merged.logical_day_complete fallback    — overnight grace window
-- CHANGE 3: flags.has_late                          — exclude PARTTIME
...
BEGIN
  ...
  attendance_day_rows AS (
    SELECT
      ...
      -- CHANGE 1: Overnight grace window until 08:00 WITA for all outlets.
      -- A non-24h day is only "complete" after 08:00 on the following day.
      -- This prevents shift SORE workers (00:00–07:59 clock-out) from being
      -- wrongly flagged as 'belum_absen_pulang' before they've actually left.
      bool_and(
        CASE
          ...
          WHEN cr.operating_mode <> 'TWENTY_FOUR_HOUR'::public.outlet_operating_mode
              AND business_today > cr.logical_date
              AND NOT (
                business_today = cr.logical_date + 1
                AND business_now_local::time < '08:00:00'::time
              ) THEN true
          ELSE false
        END
  ...
  merged AS (
    SELECT
      ...
      -- CHANGE 2: Same overnight grace applied to the schedule-only fallback
      -- (employees with a schedule entry but no attendance scans yet).
      COALESCE(
        ar.logical_day_complete,
        CASE
          WHEN mk.logical_date < business_today
              AND NOT (
                business_today = mk.logical_date + 1
                AND business_now_local::time < '08:00:00'::time
              ) THEN true
          ELSE false
        END
  ...
  CROSS JOIN LATERAL (
    SELECT
      ...
      -- CHANGE 3: PARTTIME employees are never flagged as late.
      -- They have flexible start times (e.g., shift siang can arrive at 13:00
      -- or 14:00), so the fixed late_cutoff_minutes doesn't apply to them.
      (
        COALESCE(m.relevant_event_count, 0) > 0
        AND m.employee_contract <> 'PARTTIME'::public.employee_contract
