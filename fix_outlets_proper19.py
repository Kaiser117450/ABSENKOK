...
if "_buildOutletShimmer" not in content:
    # Just look for `Widget build(BuildContext context)` in `_AdminOutletsScreenState` and put it right before that.
...
# Now, wait, there are other `Widget build` methods. The first one will be `_AdminOutletsScreenState`.
# We need to be careful to inject this exactly once.
