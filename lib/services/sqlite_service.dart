...
class SqliteService {
  ...
  /// Migration from v1 (with selfie columns) to v2 (without selfie).
  /// Since this is a local offline queue with best-effort data,
  /// it's safe to drop and recreate the table.
  static Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      await db.execute('DROP TABLE IF EXISTS pending_logs');
      await _initSchema(db, newVersion);
    } else if (oldVersion < 3) {
      // v3: CHECK constraint cannot be altered in SQLite — recreate table,
      // migrating any unsynced pending rows first.
      await db.execute('ALTER TABLE pending_logs RENAME TO pending_logs_old');
      await _initSchema(db, newVersion);
      await db.execute('''
        INSERT INTO pending_logs SELECT * FROM pending_logs_old
        WHERE sync_status IN ('pending','failed')
      ''');
      await db.execute('DROP TABLE pending_logs_old');
    } else if (oldVersion < 4) {
      // v4: Add is_backup and notes columns
      await db.execute('ALTER TABLE pending_logs ADD COLUMN is_backup INTEGER NOT NULL DEFAULT 0');
      await db.execute('ALTER TABLE pending_logs ADD COLUMN notes TEXT');
    } else if (oldVersion < 5) {
      // v5: Update CHECK constraint untuk sakit/izin - recreate table
      await db.execute('ALTER TABLE pending_logs RENAME TO pending_logs_old');
      await _initSchema(db, newVersion);
      await db.execute('''
        INSERT INTO pending_logs 
        (local_id, employee_id, scan_outlet_id, type, lat, lng, device_id, scanned_at, sync_status, retry_count, is_backup, notes, created_at)
        SELECT local_id, employee_id, scan_outlet_id, type, lat, lng, device_id, scanned_at, sync_status, retry_count, is_backup, notes, created_at
        FROM pending_logs_old
        WHERE sync_status IN ('pending','failed')
      ''');
      await db.execute('DROP TABLE pending_logs_old');
    }
